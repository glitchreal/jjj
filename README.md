...
Every field now uses one invisible, anchored, client-created `20 x 1 x 20` platform instead of close-range ground orbit. The platform targets about 10 studs above the newest live Vicious attack marker, prioritizing the real top-level `Workspace.Particles.WarningDisk` and then `Workspace.Particles.Thorn`; it falls back to the Vicious activation part or the living monster when no attack marker exists. A smoothing rate of 8 and a four-stud maximum update step follow moving targets without snapping. Five-stud correction and 1.5-stud settle thresholds bring the unanchored character to the platform and then release `AlignPosition` and noclip so the character can stand and walk normally.
...
While hunting, one guarded `Workspace.DescendantAdded` listener prioritizes those exact top-level attack objects and retains combined ancestry, recent-creation, proximity, and geometry checks as a compatibility fallback. Their current positions are sampled while they exist, with the Thorn's base used rather than its tall mesh center. The exact warning-to-damage lifetime was not measured before the observed Vicious died, so markers remain eligible for a conservative five-second window.
