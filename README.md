...
The killer claims a hive after its character finishes loading. It inspects `Workspace.HivePlatforms`, follows each platform's `Hive` ObjectValue to the corresponding `Workspace.Honeycombs` model, and treats a platform as available only when both `PlayerRef` and the Honeycomb's `Owner` are empty and its phase is `Idle`. The character Tweens to the nearest available platform, occupancy is checked again, and then `ReplicatedStorage.Events.ClaimHive` is fired with that Honeycomb's `HiveID`. A bounded checker verifies ownership from both `PlayerRef` and `Owner`, retries delayed remote replication, and rescans for another empty hive if the selected one becomes occupied or the approach fails. It allows up to five attempts within the claim window before continuing to the Vicious target.

After a claimed server is validated and a living Vicious Bee is confirmed, the killer creates one `AlignPosition` and one `AlignOrientation` on HumanoidRootPart. If `Workspace.Particles.Vicious` exists, activation continuously follows that live part, first moving eight studs above its top and then dropping vertically to five studs below its bottom. The root therefore sweeps through the moving spike instead of approaching it sideways. Native `Touched` contact is preferred, with a bounds-checked executor touch fallback while overlapping and repeated vertical sweeps if contact is missed. Combat then switches directly to the universal overhead platform described below.

Every field now uses one invisible, anchored, client-created `20 x 1 x 20` platform instead of close-range ground orbit. The platform normally stays about 10 studs above the living Vicious Bee. Top-level `Workspace.Particles.WarningDisk` and `Workspace.Particles.Thorn` attacks are exclusion zones, never destinations: when one appears near the platform, the controller selects the nearest safe point on expanding rings around the Vicious Bee while maintaining an eight-stud danger radius plus three studs of clearance. A smoothing rate of 8 and a four-stud maximum update step move away without snapping. Five-stud correction and 1.5-stud settle thresholds bring the unanchored character to the platform and then release `AlignPosition` and noclip so the character can stand and walk normally.

Only the platform is anchored. It is transparent, non-touching, non-queryable, and exists only in the local client, so it is not replicated to or controlled by other players. A 512-stud downward surface scan and a local collision-volume check raise the destination above terrain or cave roofs before movement, preventing the King Beetle cave and similar geometry from trapping the character. If the platform cannot be created or is removed, a bounded-speed overhead hover takes over and retries platform creation once per second. The platform is destroyed with the movement constraints when the Vicious Bee dies or disappears, the character changes, the claim is lost, teleport begins, the runtime is replaced, or the script ends.
...
While hunting, one guarded `Workspace.DescendantAdded` listener prioritizes those exact top-level attack objects and retains combined ancestry, recent-creation, proximity, and geometry checks as a compatibility fallback. Their current positions are sampled while they exist, with the Thorn's base used rather than its tall mesh center. A live level-six attack sequence measured many ground attacks: red `WarningDisk` markers lasted approximately 1.95-2.27 seconds, while the corresponding `Thorn` objects lasted approximately 3.33-3.66 seconds. Destroyed attack objects are removed from avoidance immediately; five seconds remains the conservative fallback for unexpectedly retained objects.

The movement controller disconnects its listeners, destroys its platform, constraints, and attachment, restores every modified collision value and Humanoid AutoRotate state, and stops walking when the target dies or disappears, the claim is lost, teleport starts, the runtime is replaced, or the script exits. Live MCP inspection reconfirmed `Workspace.Particles.Vicious` as a non-collidable, touch-enabled activation `Part`. The activation controller continuously follows its live position, approaches above it, and drops vertically through it before switching to overhead combat. Spike geometry and timing remain a compatibility risk after Bee Swarm updates.
